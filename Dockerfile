...
RUN chmod +x ./usr/local/bin/docker-entrypoint.sh ./bin/bash /docker-entrypoint.sh
ENTRYPOINT ["./usr/local/bin/docker-entrypoint.sh ./bin/bash /docker-entrypoint.sh"]
